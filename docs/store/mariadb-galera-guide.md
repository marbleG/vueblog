...
### 5. 全局关机后集群恢复（所有节点全部关闭）Docker 部署

**场景描述：**当使用 Docker 部署的 MariaDB Galera Cluster 所有节点因为维护、断电或其他原因全部关闭后，需要重新启动集群。这是生产环境中常见但关键的恢复场景。

#### ⚠️ 恢复原则

1. **确定启动节点**：选择一个节点作为主引导节点
2. **数据安全**：确保最后一个关闭的节点拥有最新数据
3. **有序启动**：一次启动一个节点，等待完全同步后再启动下一个

---

### Docker 部署场景

当使用 Docker 部署 MariaDB Galera Cluster 时，自动恢复机制与裸机部署有所不同。以下是 Docker 环境下的完整实现方案。
...
services:
    ...
    command: >
      bash -c "
        if [ ! -f /var/lib/mysql/mysql ]; then
          ...
          mysqld --user=root --wsrep-new-cluster
        else
  ...
  galera-node2:
    ...
    ports:
      ...
      "4444:4444"
    volumes:
      - galera-data2:/var/lib/mysql
      - ./galera-node2.cnf:/etc/mysql/conf.d/galera.cnf:ro
    environment:
      - MYSQL_ROOT_PASSWORD=root_password
      - CLUSTER_NAME=my_galera_cluster
      - WSREP_CLUSTER_ADDRESS=gcomm://galera-node1,galera-node3
      - WSREP_NODE_NAME=galera-node2
      - WSREP_NODE_ADDRESS=galera-node2
    command: mysqld
    depends_on:
      - galera-node1
    restart: unless-stopped
  ...
  galera-node3:
    ...
    ports:
      ...
      - "4444:4444"
    volumes:
      - galera-data3:/var/lib/mysql
      - ./galera-node3.cnf:/etc/mysql/conf.d/galera.cnf:ro
    environment:
      - MYSQL_ROOT_PASSWORD=root_password
      - CLUSTER_NAME=my_galera_cluster
      - WSREP_CLUSTER_ADDRESS=gcomm://galera-node1,galera-node2
     WSREP_NODE_NAME=galera-node3
      - WSREP_NODE_ADDRESS=galera-node3
    command: mysqld
    depends_on:
      - galera-node1
    restart: unless-stopped
...
```bash
mkdir -p galera-docker-cluster
cd galera-docker-cluster
```
...
**创建其他节点的配置文件（galera-node2.cnf, galera-node3.cnf）**，只需修改：

- `wsrep_node_name`：galera-node2 / galera-node3
- `wsrep_node_address`：galera-node2 / galera-node3
- `wsrep_cluster_address`：包含其他两个节点
...
docker-compose logs -f galera-node3

# 测试集群
docker exec -it galera-node1 mysql -u root -p
```
...
services:
  galera-node1:
    healthcheck:
      test: ["CMD", "mysqladmin", "-u", "root", "-p", "root_password", "ping"]
      interval: 10s
      timeout: 5s
      retries: 5
      start_period: 30s
...
services:
  galera-node1:
    restart: unless-stopped  # 容器停止时不自动重启
```
...
# 检查集群状态
docker exec -it galera-node1 mysql -u root -p -e "SHOW STATUS LIKE 'wsrep%';"
```
...
```bash
# 查看详细日志
docker-compose logs galera-node1

# 检查配置文件
docker exec -it galera-node1 cat /etc/mysql/conf.d/galera.cnf

# 检查数据卷权限
docker volume inspect galera-data1
```
...
```bash
# 检查网络连通性
docker exec -it galera-node1 ping galera-node2

# 检查端口
docker exec -it galera-node1 netstat -tlnp | grep -E '3306|4567'

# 查看 Galera 日志
docker exec -it galera-node1 cat /var/log/mysql/error.log
```
